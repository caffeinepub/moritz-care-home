...
export function getBackendDiagnostics(): BackendDiagnostics {
  ...
  const environment = dfxNetwork || (resolvedHost.includes('localhost') ? 'local' : 'ic');
 ...
 */
export async function performHealthCheck(timeoutMs = 5000): Promise<HealthCheckResult> {
  ...
  try {
    ...
    const actor = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('Actor creation timed out')), timeoutMs)
      ),
    ]);
    ...
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error('Health check call timed out')), timeoutMs)
      ),
    ]);
    ...
    if (rawMessage.includes('is stopped') || rawMessage.includes('canister is stopped')) {
      return {
        status: 'failed',
        success: false,

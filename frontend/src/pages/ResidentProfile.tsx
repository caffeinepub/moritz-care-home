...
export default function ResidentProfile() {
  ...
  const activeMedications = resident.medications.filter((m) => String(m.status) === 'active');
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 print:hidden">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center space-x-2">
            <Switch id="signature-mode" checked={includeSignature} onCheckedChange={setIncludeSignature} />
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-1">
              ...
              <p className="font-medium">{resident.roomType === 'solo' ? 'Solo' : 'Shared Room'}</p>
            </div>
            <div className="space-y-1">
              ...
              {activeMedications.map((medication) => (
                <Card key={String(medication.id)}>
                  ...
                  <CardContent className="space-y-2">
                    ...
                    {!adminLoading && isAdmin && (
                      <EditMedicationDialog resident={resident} medication={medication}>
                        <Button variant="outline" size="sm" onClick={() => handleEditMedication(medication)} className="mt-2 w-full">
                          <Edit className="mr-2 h-4 w-4" />
                          Edit Medication
                        </Button>
                    ...
                    <div className="grid gap-2 md:grid-cols-3 lg:grid-cols-6 text-sm">
                      ...
                      {vitals.bloodGlucose != null && (
                        <div>
                          ...
                          <p className="font-medium">{vitals.bloodGlucose} mg/dL</p>
                        </div>
                      )}
                    </div>
                    {vitals.notes && (
                      <p className="mt-2 text-sm text-muted-foreground">{vitals.notes}</p>
                    )}
                  </CardContent>
                </Card>
              ))}
                  ...
                  <CardContent className="pt-4">
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          {record.medication.dosage} — {String(record.medication.administrationRoute)}
                        </p>
                      </div>
              ...
              {[...resident.adlRecords].reverse().map((record) => (
                <Card key={String(record.id)}>
                  <CardContent className="pt-4">
                    ...
                    {record.staffNotes && (
                      <p className="mt-2 text-sm text-muted-foreground">{record.staffNotes}</p>
                    )}
                  </CardContent>
                </Card>
              ))}
              ...
              {[...resident.weightLog].reverse().map((entry) => (
                <Card key={String(entry.id)}>
                  <CardContent className="pt-4">
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium">{entry.weight} {entry.weightUnit}</p>
                      </div>
                      <p className="text-sm text-muted-foreground">{formatDate(entry.measurementDate)}</p>
                    </div>
                    {entry.notes && (

...
export default function Dashboard() {
  ...
  const handleDischargeConfirm = async () => {
    ...
    try {
      ...
      toast.success(`${selectedResident.firstName} ${selectedResident.lastName} has been discharged.`);
      setDischargeConfirmOpen(false);
      setSelectedResident(null);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to discharge resident.');
    }
  };
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toast.success(`${selectedResident.firstName} ${selectedResident.lastName} has been deleted.`);
      setDeleteConfirmOpen(false);
      setSelectedResident(null);
    } catch (error) {
      toast.error(error instanceof Error ? error.message : 'Failed to delete resident.');
    }
  };
  ...
  const renderResidentCard = (resident: Resident) => {
    ...
    return (
      <Card key={String(resident.id)} className="cursor-pointer transition-shadow hover:shadow-lg">
            ...
            <div className="mt-4 flex gap-2">
              {isActive && (
                <Button
                  variant="outline"
                  size="sm"
                  onClick={(e) => { e.stopPropagation(); handleDischargeClick(resident); }}
              ...
              <Button
                variant="destructive"
                size="sm"
                onClick={(e) => { e.stopPropagation(); handleDeleteClick(resident); }}
  ...
  if ((activeError || dischargedError || adminError) && (activeFetched || dischargedFetched)) {
    const errorMessage =
      activeError?.message || dischargedError?.message || adminError?.message || 'Failed to load residents';
    return (
    ...
    <div className="container mx-auto space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage and view all residents in the care home</p>
        </div>
        {!adminLoading && isAdmin && (
          <AddResidentDialog>
            <Button size="lg">
        ...
        <TabsList>
          <TabsTrigger value="active" className="flex items-center gap-2">
            ...
            {activeFetched && !loadingActive && (
              <Badge variant="secondary" className="ml-1">{activeResidents.length}</Badge>
            )}
          </TabsTrigger>
          <TabsTrigger value="discharged" className="flex items-center gap-2">
            ...
            {dischargedFetched && !loadingDischarged && (
              <Badge variant="secondary" className="ml-1">{dischargedResidents.length}</Badge>
            )}
          </TabsTrigger>
        </TabsList>
          ...
          {loadingDischarged ? (
            ...
            <Card>
              <CardContent className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-muted-foreground">Discharged residents will appear here</p>
              </CardContent>
            </Card>
          ) : (
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      ...
      <AlertDialog open={dischargeConfirmOpen} onOpenChange={setDischargeConfirmOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={handleDischargeConfirm} disabled={dischargeMutation.isPending}>

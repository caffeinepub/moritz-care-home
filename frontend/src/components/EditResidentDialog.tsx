...
import { mapRoomTypeToBackend, mapRoomTypeFromBackend } from '../lib/residentEnumMapping';
...
export default function EditResidentDialog({ resident, children }: EditResidentDialogProps) {
  ...
  const [physicians, setPhysicians] = useState<{ id: number; name: string; specialty: string; contactNumber: string }[]>([]);
  const [responsiblePersons, setResponsiblePersons] = useState<{ id: number; name: string; relationship: string; contactNumber: string; address: string }[]>([]);
  const [medications, setMedications] = useState<{ id: number; name: string; dosage: string; administrationTimes: string[]; administrationRoute: string; dosageQuantity: string; notes: string; status: string }[]>([]);
  ...
  useEffect(() => {
    if (open && resident) {
      ...
      setPhysicians(resident.physicians.map((p) => ({ ...p, id: Number(p.id) })));
      setResponsiblePersons(resident.responsiblePersons.map((rp) => ({ ...rp, id: Number(rp.id) })));
      setMedications(resident.medications.map((m) => ({
        id: Number(m.id),
        name: m.name,
        dosage: m.dosage,
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input type="date" value={dateOfBirth} onChange={(e) => setDateOfBirth(e.target.value)} className="bg-white" />
            </div>
            <div className="space-y-2">
              <Label>Admission Date</Label>
              <Input type="date" value={admissionDate} onChange={(e) => setAdmissionDate(e.target.value)} className="bg-white" />
            </div>
            <div className="space-y-2">
              <Label>Room Number *</Label>
              <Input value={roomNumber} onChange={(e) => setRoomNumber(e.target.value)} className="bg-white" />
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label>Bed</Label>
              <Input value={bed} onChange={(e) => setBed(e.target.value)} className="bg-white" />
            </div>
            <div className="space-y-2">
              <Label>Medicaid Number</Label>
              <Input value={medicaidNumber} onChange={(e) => setMedicaidNumber(e.target.value)} className="bg-white" />
            </div>
            <div className="space-y-2">
              <Label>Medicare Number</Label>
              <Input value={medicareNumber} onChange={(e) => setMedicareNumber(e.target.value)} className="bg-white" />
            </div>
          </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => setPhysicians([...physicians, { id: Date.now(), name: '', specialty: '', contactNumber: '' }])}>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => setResponsiblePersons([...responsiblePersons, { id: Date.now(), name: '', relationship: '', contactNumber: '', address: '' }])}>
                <Plus className="h-3 w-3 mr-1" /> Add
              </Button>
            </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button type="button" variant="outline" size="sm" onClick={() => setMedications([...medications, { id: Date.now(), name: '', dosage: '', administrationTimes: [], administrationRoute: 'oral', dosageQuantity: '', notes: '', status: 'active' }])}>
                <Plus className="h-3 w-3 mr-1" /> Add
              </Button>
            </div>
            {medications.map((m, i) => (
              <div key={m.id} className="grid grid-cols-2 gap-2 p-3 border rounded-lg bg-white">
                ...
                <Input placeholder="Admin Times (comma-separated)" value={m.administrationTimes.join(', ')} onChange={(e) => setMedications(medications.map((x, j) => j === i ? { ...x, administrationTimes: e.target.value.split(',').map((t) => t.trim()).filter(Boolean) } : x))} className="bg-white" />

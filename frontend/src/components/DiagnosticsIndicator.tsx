          ...
          <div className="space-y-1 text-xs">
            <div className="flex justify-between gap-4">
              ...
              <span className="font-mono text-right break-all">
                {hasCanisterId ? diagnostics.canisterId : (
            ...
            <div className="flex justify-between gap-4">
              ...
              <span className="font-mono text-right">{diagnostics.environment}</span>
            </div>
          </div>
            ...
            <div className="border-t border-border pt-2 space-y-1 text-xs">
              ...
              {buildInfo.buildId && (
                <div className="flex justify-between gap-4">
                  ...
                  <span className="font-mono text-right">{buildInfo.buildId}</span>
                </div>
              )}
              {buildInfo.buildTime && (
                <div className="flex justify-between gap-4">
                  <span className="text-muted-foreground">Built</span>
                  <span className="font-mono text-right">{buildInfo.buildTime}</span>
                </div>
              )}
            </div>
          )}
          ...
          {!hasMetadata && (
            <div className="border-t border-border pt-2">
              <p className="text-xs text-muted-foreground">
                Missing build metadata — deploy via <code className="bg-muted px-1 rounded">redeploy_production.sh</code> to inject build info.
              </p>
            </div>
          )}
          ...
          {!hasCanisterId && (
            <div className="border-t border-border pt-2">
              <p className="text-xs text-warning">
                Canister ID not found in environment variables. Check Vite config and deployment setup.
              </p>
            </div>
          )}
        </TooltipContent>

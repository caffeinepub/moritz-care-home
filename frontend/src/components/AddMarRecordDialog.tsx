...
export default function AddMarRecordDialog({ resident, children }: AddMarRecordDialogProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedMedicationId || !administrationTime || !administeredBy.trim()) {
    ...
    addMarRecord(
      ...
      },
      {
        ...
        onError: (err) => {
          toast.error(err instanceof Error ? err.message : 'Failed to add MAR record.');
        },
      }
    );
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {children ?? <Button variant="outline" size="sm">Add MAR Record</Button>}
      </DialogTrigger>

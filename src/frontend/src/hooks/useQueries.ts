...
export function useGetCallerUserProfile() {
  ...
  const query = useQuery<UserProfileWithRole | null>({
    ...
    queryFn: async () => {
      ...
      const timeoutPromise = new Promise<never>((_, reject) =>
        setTimeout(
          () =>
            reject(
              new Error(
                `Profile query timed out after ${PROFILE_QUERY_TIMEOUT}ms`,
              ),
            ),
      ...
      if (
        result.__kind__ === "err" &&
        result.err === ProfileSetupError.NotFound
      ) {
        return null;
      }
      return null;
...
export function useGetActiveResidents() {
  ...
  return useQuery<Resident[]>({
    ...
    queryFn: async () => {
      ...
      const result = await (
        actor as any
      ).getNonArchivedResidentsAccessControl();
      const residents: Resident[] = (result || []).map((r: any) =>
        normalizeResident(r),
      );
...
export function useDischargeResident() {
  ...
  return useMutation({
    mutationFn: async (residentId: number) => {
      if (!actor) throw new Error("Actor not available");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (actor as any).v105_dischargeResidentAccessControl(
        BigInt(residentId),
      );
    },
    onSuccess: () => {
...
export function usePermanentlyDeleteResident() {
  ...
  return useMutation({
    mutationFn: async (residentId: number) => {
      if (!actor) throw new Error("Actor not available");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (actor as any).v105_permanentlyDeleteResidentAccessControl(
        BigInt(residentId),
      );
    },
    onSuccess: () => {
  ...
  return useMutation({
    ...
    onSuccess: (_data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["resident", variables.residentId],
      });
      queryClient.invalidateQueries({ queryKey: ["residents"] });
    },
  });
...
export function useAddDailyVitals() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (actor as any).addDailyVitalsAccessControl(
        BigInt(data.residentId),
        {
          residentId: BigInt(data.residentId),
          ...
          bloodGlucose:
            data.bloodGlucose != null ? [BigInt(data.bloodGlucose)] : [],
...
export function useAddWeightEntry() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (actor as any).addWeightEntryAccessControl(
        BigInt(data.residentId),
        {
          residentId: BigInt(data.residentId),
          weight: data.weight,
...
export function useUpdateMedication() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (actor as any).updateMedicationAccessControl(
        BigInt(data.residentId),
        {
...
export function useUpdateResident() {
  ...
  return useMutation({
    mutationFn: async (data: {
      ...
      if (!actor) throw new Error("Actor not available");
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await (actor as any).updateResidentAccessControl(
        BigInt(data.residentId),
        data.updates,
      );
    },
...
function normalizeResident(r: any): Resident {
  return {
    ...
    lastName: r.lastName,
    dateOfBirth:
      typeof r.dateOfBirth === "bigint" ? Number(r.dateOfBirth) : r.dateOfBirth,
    admissionDate:
      typeof r.admissionDate === "bigint"
        ? Number(r.admissionDate)
        : r.admissionDate,
    ...
    pharmacy:
      r.pharmacy?.__kind__ === "Some"
        ? r.pharmacy.value
        : (r.pharmacy ?? undefined),
    insurance:
      r.insurance?.__kind__ === "Some"
        ? r.insurance.value
        : (r.insurance ?? undefined),
    ...
    marRecords: (r.marRecords ?? []).map((m: any) => ({
      ...
      administrationTime:
        typeof m.administrationTime === "bigint"
          ? Number(m.administrationTime)
    ...
    adlRecords: (r.adlRecords ?? []).map((a: any) => ({
      ...
      residentId:
        typeof a.residentId === "bigint" ? Number(a.residentId) : a.residentId,
    ...
    dailyVitals: (r.dailyVitals ?? []).map((v: any) => ({
      ...
      residentId:
        typeof v.residentId === "bigint" ? Number(v.residentId) : v.residentId,
      measurementDateTime:
        typeof v.measurementDateTime === "bigint"
          ? Number(v.measurementDateTime)
          : v.measurementDateTime,
      bloodPressureSystolic:
        typeof v.bloodPressureSystolic === "bigint"
          ? Number(v.bloodPressureSystolic)
          : v.bloodPressureSystolic,
      bloodPressureDiastolic:
        typeof v.bloodPressureDiastolic === "bigint"
          ? Number(v.bloodPressureDiastolic)
          : v.bloodPressureDiastolic,
      pulseRate:
        typeof v.pulseRate === "bigint" ? Number(v.pulseRate) : v.pulseRate,
      respiratoryRate:
        typeof v.respiratoryRate === "bigint"
          ? Number(v.respiratoryRate)
          : v.respiratoryRate,
      oxygenSaturation:
        typeof v.oxygenSaturation === "bigint"
          ? Number(v.oxygenSaturation)
          : v.oxygenSaturation,
      bloodGlucose:
        v.bloodGlucose != null
          ? typeof v.bloodGlucose === "bigint"
            ? Number(v.bloodGlucose)
            : v.bloodGlucose
          : undefined,
    })),
    weightLog: (r.weightLog ?? []).map((w: any) => ({
      ...
      residentId:
        typeof w.residentId === "bigint" ? Number(w.residentId) : w.residentId,
      measurementDate:
        typeof w.measurementDate === "bigint"
          ? Number(w.measurementDate)
          : w.measurementDate,
    })),
    dischargeTimestamp:
      r.dischargeTimestamp?.__kind__ === "Some"
